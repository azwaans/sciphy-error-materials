...
num_datasets <- 50
if((analysis_name == "both") && (sampling_proportion == '0.05')) {
  num_datasets <- 100
}
...
parameters_of_interest = c("clockRate", paste0("insertRate.", 1:13),"missRate","missProb")
...
clock_rate_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)

insert_rate_inference = data.frame(seed=rep(1:num_datasets, each = 13), hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F,
                                   insertRate= rep(1:13,100))

miss_rate_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)

miss_prob_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)

birth_rate_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)

death_rate_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)


growth_rate_array_5_inference = data.frame(seed=1:num_datasets, hpd_lower=0, hpd_upper=0, median=0, true_value=0, recovered=F)

for (seed in  1:num_datasets){

  print(seed)
  # get inference log
  log_file = paste0("infer_subsampled_simulated_",analysis_name,"_",sampling_proportion,"_",targets,"targets.seed=", seed, ".log")
  if(! file.exists(paste0(log_dir, log_file))) {
    print(paste0(log_dir, log_file))
    print("doesn't exist")
    next}
  log_data = parse_beast_tracelog_file(paste0(log_dir, log_file))
  if(length(log_data$Sample) == 1) {
    print(paste0(log_dir, log_file))
    print("stopped")
    next
  }
  ...
  if(analysis_name == "50_heritable") {
    esses = esses[! names(esses) %in% c("samplingProportion", "treeHeight.t.alignment", "treeLength.t.alignment","missProb",paste0("treeLikelihood_",1:20))]
  }
  if(analysis_name == "50_dropout") {
    esses = esses[! names(esses) %in% c("samplingProportion", "treeHeight.t.alignment", "treeLength.t.alignment","missRate",paste0("treeLikelihood_",1:20))]
  }
  ...
  ### for the missRate
  miss_rate_array_5_hpd = hdi(object = log_data_wo_burnin$missRate, credMass = 0.95)
  
  ### extract simulation value
  true_miss_rate = simulation_parameters[15, 2]
  ...
  miss_rate_array_5_inference[seed, ] = c(seed, miss_rate_array_5_hpd, median(log_data_wo_burnin$missRate), true_miss_rate, recovered)
  ...
  ### for the missProb
  miss_prob_array_5_hpd = hdi(object = log_data_wo_burnin$missProb, credMass = 0.95)
  
  ###
  true_miss_prob = simulation_parameters[16, 2]
  ...
  print(recovered)
  miss_prob_array_5_inference[seed, ] = c(seed, miss_prob_array_5_hpd, median(log_data_wo_burnin$missProb), true_miss_prob, recovered)
...
clock_rate_array_5_inference$orderedSeed = 1:num_datasets
...
miss_rate_array_5_inference$orderedSeed = 1:num_datasets
...
write.csv(x = clock_rate_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_clock_rate_inference_with_error.csv"),quote = F, row.names = F)
write.csv(x = miss_rate_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_miss_rate_inference_with_error.csv"),quote = F, row.names = F)
write.csv(x = miss_prob_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_miss_prob_inference_with_error.csv"),quote = F, row.names = F)

write.csv(x = birth_rate_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_","birth_rate_inference_with_error.csv"),quote = F, row.names = F)
write.csv(x = death_rate_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_","death_rate_inference_with_error.csv"),quote = F, row.names = F)
write.csv(x = growth_rate_array_5_inference, paste0(analysis_name,"_",sampling_proportion,"_","growth_rate_inference_with_error.csv"),quote = F, row.names = F)
